...
func TestDocsHaveNoRelativeLinks(t *testing.T) {
	err := filepath.WalkDir("docs", func(path string, entry fs.DirEntry, err error) error {
		...
		for _, match := range markdownLink.FindAllStringSubmatch(string(content), -1) {
			...
			t.Errorf("%s links to %q, which resolves one segment too high on the registry and 404s; "+
				"use a full https:// URL or drop the link and let the registry sidebar do the navigating", path, target)
		}
		return nil
	})
	if err != nil {
